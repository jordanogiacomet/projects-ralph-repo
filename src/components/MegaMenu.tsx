    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute left-0 right-0 top-full z-50 pt-3 lg:pt-4"
        ...
        >
          <div className="relative overflow-hidden rounded-[36px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(248,250,252,0.98)_100%)] shadow-[0_40px_90px_rgba(15,23,42,0.18)] backdrop-blur-xl">
            <div className="relative max-h-[calc(100svh-7.75rem)] overflow-y-auto overscroll-contain">
              <div
                aria-hidden="true"
              ...
              <div className="relative grid border-b border-border/70 lg:grid-cols-[minmax(0,1fr)_minmax(240px,0.68fr)]">
                <div className="p-5 sm:p-6 lg:p-6 xl:p-7">
                  ...
                  <h2 className="mt-4 max-w-2xl font-display text-heading-xl font-semibold tracking-tight text-text-primary sm:text-heading-2xl">
                    {title || 'Navegação principal'}
                  </h2>
                  <p className="mt-2.5 max-w-xl text-sm leading-6 text-text-secondary">
                ...
                <div className="border-t border-border/70 bg-[linear-gradient(160deg,rgba(234,242,251,0.72)_0%,rgba(255,255,255,0.92)_100%)] p-5 lg:border-l lg:border-t-0 lg:p-6 xl:p-7">
                    ...
                    {items.slice(0, 3).map((item, index) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={onClose}
                          className={cn(
                            'motion-transition motion-lift-subtle rounded-[20px] border px-4 py-3.5 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/15',
                            isItemActive
                  ...
                  {items.map((category, index) => {
                    ...
                    return (
                      <article
                        ...
                        className={cn(
                          'group motion-transition motion-transition-emphasis motion-lift-card rounded-[24px] border p-4 sm:p-5',
                          isCategoryActive
                      ...
                      >
                          ...
                          <div className="min-w-0 flex-1">
                              ...
                              <div>
                                {category.link ? (
                                  <Link
                                    href={category.link}
                                    onClick={onClose}
                                    className="motion-transition text-base font-semibold tracking-[-0.03em] text-text-primary hover:text-accent sm:text-lg"
                                    role="menuitem"
                                  >
                                    {category.label}
                                  </Link>
                                ) : (
                                  <h3 className="text-base font-semibold tracking-[-0.03em] text-text-primary sm:text-lg">
                                    {category.label}
                                  </h3>
                                )}
                            ...
                            {category.link ? (
                              <Link
                                href={category.link}
                                onClick={onClose}
                                className="motion-transition motion-lift-subtle mt-3.5 inline-flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent-hover focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/15"
                                role="menuitem"
                              >
                        ...
                        {category.children && category.children.length > 0 ? (
                          <ul className="mt-4 space-y-2 border-t border-border/70 pt-3.5">
                                ...
                                <li key={child.id || child.label}>
                                  {child.link ? (
                                    <Link
                                      ...
                                      onClick={onClose}
                                      className={cn(
                                        'motion-transition motion-lift-subtle flex items-center justify-between gap-3 rounded-[18px] border px-3 py-2.5 text-sm font-medium focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/15',
                                        isChildActive
