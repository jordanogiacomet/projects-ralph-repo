...
const solutionSlug = 'combinacao-de-negocios-ppa-cpc-15'
const legacySolutionSlug = 'combinacao-de-negocios'
      ...
      children: [
        {
          type: 'heading',
          tag: 'h2',
          children: [{ type: 'text', text: 'Estruturação técnica para processos de PPA com segurança contábil' }],
        },
        {
          type: 'paragraph',
          children: [
...
async function getSolutionPageData(): Promise<SolutionPageData> {
  try {
    ...
    const title = normalizeText(solution.title) || fallbackData.title
    const normalizedTitleLower = title.toLowerCase()
    const heroTitle =
      normalizedTitleLower.includes('cpc 15') || normalizedTitleLower.includes('ppa')
        ? title
        : `${title} (PPA) – CPC 15`
    const breadcrumbTitle =
      normalizedTitleLower.includes('combinação de negócios') ||
      normalizedTitleLower.includes('combinacao de negocios')
        ? fallbackData.breadcrumbTitle
        : title
    const description =
    ...
    return {
      ...
      metaTitle: normalizeText(solution.meta?.title) || `${heroTitle} | Apollo Gestão`,
...
export default async function CombinacaoDeNegociosPpaCpc15Page() {
  const data = await getSolutionPageData()
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl px-4 py-20 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-6 text-sm text-white/80">
            <ol className="flex flex-wrap items-center gap-2">
              ...
              <li aria-hidden>›</li>
              <li>
                <Link href="/solucoes/consultoria-e-tecnologia" className="hover:text-white">
                  {data.categoryTitle}
                </Link>
              </li>
              <li aria-hidden>›</li>
              <li className="text-white">{data.breadcrumbTitle}</li>
            </ol>
          </nav>
