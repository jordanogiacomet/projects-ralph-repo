...
const solutionSlug = 'arrendamento-mercantil'
const legacySolutionSlug = 'arrendamento-mercantil-cpc-06-ifrs-16'
      ...
      children: [
        {
          type: 'heading',
          tag: 'h2',
          children: [{ type: 'text', text: 'Conformidade técnica para contratos de arrendamento' }],
        },
        {
          type: 'paragraph',
          children: [
...
async function getSolutionPageData(): Promise<SolutionPageData> {
  try {
    ...
    const heroTitle =
      title.toLowerCase().includes('cpc 06') || title.toLowerCase().includes('ifrs 16')
        ? title
        : `${title} - CPC 06 (IFRS 16)`
    const description =
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/solucoes/${categorySlug}/arrendamento-mercantil`,
    },
    openGraph: {
      title: data.metaTitle,
      description: data.metaDescription,
      type: 'article',
      url: `/solucoes/${categorySlug}/arrendamento-mercantil`,
    },
  }
}

export default async function ArrendamentoMercantilPage() {
  const data = await getSolutionPageData()
